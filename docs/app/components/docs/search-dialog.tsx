...
function HighlightedText({ text: raw, query }: { text: string; query: string }) {
  // Fumadocs marks matches with <mark>; strip them and highlight here instead.
  const text = raw.replace(/<\/?mark>/g, '').replace(/`/g, '')
  if (!query.trim()) {
    return <>{text}</>
  }
...
function SearchDialog({ onClose }: SearchDialogProps) {
  ...
  const { search, setSearch, query } = useDocsSearch({ type: 'static', from: withBasePath('/api/search') })
