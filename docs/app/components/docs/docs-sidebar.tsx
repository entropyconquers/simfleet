...
/** Static export uses trailing slashes; page-tree URLs do not. */
export function normalizePath(pathname: string): string {
  return pathname.length > 1 ? pathname.replace(/\/+$/, '') : pathname
}

/** The API reference tab shows only its own pages; the docs tab hides them. */
export function nodesForTab(tree: Root, pathname: string): Node[] {
  const isApi = (node: Node) =>
    node.type === 'folder' &&
    (node.index?.url.includes('/api-reference') ||
      node.children.some((child) => child.type === 'page' && child.url.includes('/api-reference')))
  if (pathname.includes('/api-reference')) {
    const api = tree.children.find(isApi)
    return api && api.type === 'folder' ? [...(api.index ? [api.index] : []), ...api.children] : tree.children
  }
  return tree.children.filter((node) => !isApi(node))
}

export function DocsSidebar({ tree }: DocsSidebarProps) {
  const pathname = normalizePath(usePathname())
  const nodes = nodesForTab(tree, pathname)
  ...
  return (
    <aside className="hidden lg:block w-64 shrink-0">
      <nav className="sticky top-36 max-h-[calc(100vh-10rem)] overflow-y-auto pb-10 pr-4">
        ...
        <SidebarNodes nodes={nodes} pathname={pathname} level={0} />
      </nav>
    </aside>
  )
}
